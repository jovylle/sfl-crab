// Digging Formations Data
// Extracted from Sunflower Land game repo (src_other/features/game/types/desert.ts)
// Last updated: November 2025

import { getCurrentSeasonalArtefact } from './seasonalArtefacts.js'

// Get current seasonal artifact once at module load
const CURRENT_SEASONAL_ARTEFACT = getCurrentSeasonalArtefact()

/**
 * Treasure formation patterns for desert digging
 * Each pattern is an array of plots with relative x, y coordinates and treasure name
 */
export const DIGGING_FORMATIONS = {
  // Horizontal Zig Zag - X Coins
  MONDAY_ARTEFACT_FORMATION: [
    { x: 0, y: -1, name: "Camel Bone" },
    { x: 1, y: -1, name: CURRENT_SEASONAL_ARTEFACT },
  ],
  // Upside T - X Coins
  TUESDAY_ARTEFACT_FORMATION: [
    { x: 1, y: 0, name: CURRENT_SEASONAL_ARTEFACT },
    { x: 1, y: 2, name: "Camel Bone" },
  ],

  // Star - X Coins
  WEDNESDAY_ARTEFACT_FORMATION: [
    { x: 0, y: 0, name: "Camel Bone" },
    { x: 1, y: 0, name: CURRENT_SEASONAL_ARTEFACT },
  ],

  // Square - X Coins
  THURSDAY_ARTEFACT_FORMATION: [
    { x: 0, y: 0, name: "Camel Bone" },
    { x: 1, y: -1, name: CURRENT_SEASONAL_ARTEFACT },
  ],

  // Horizontal - X Coins
  FRIDAY_ARTEFACT_FORMATION: [
    { x: 2, y: 0, name: "Camel Bone" },
    { x: 3, y: 0, name: CURRENT_SEASONAL_ARTEFACT },
  ],

  // U Shape - X Coins
  SATURDAY_ARTEFACT_FORMATION: [
    { x: 1, y: 0, name: CURRENT_SEASONAL_ARTEFACT },
    { x: 1, y: -1, name: "Camel Bone" },
  ],

  // Horizontal - X Coins
  SUNDAY_ARTEFACT_FORMATION: [
    { x: 0, y: -2, name: CURRENT_SEASONAL_ARTEFACT },
  ],

  ARTEFACT_ONE: [
    { x: 0, y: 0, name: CURRENT_SEASONAL_ARTEFACT },
    { x: 0, y: 1, name: "Camel Bone" },
    { x: 0, y: 2, name: "Camel Bone" },
  ],

  ARTEFACT_TWO: [
    { x: 0, y: 0, name: CURRENT_SEASONAL_ARTEFACT },
    { x: 1, y: 0, name: "Camel Bone" },
    { x: 0, y: 2, name: "Camel Bone" },
  ],

  ARTEFACT_THREE: [{ x: 0, y: 0, name: CURRENT_SEASONAL_ARTEFACT }],

  ARTEFACT_FOUR: [
    { x: 0, y: 0, name: CURRENT_SEASONAL_ARTEFACT },
    { x: 0, y: 1, name: "Camel Bone" },
    { x: 0, y: 2, name: "Camel Bone" },
    { x: 0, y: 3, name: "Camel Bone" },
  ],

  ARTEFACT_FIVE: [
    { x: 0, y: 0, name: CURRENT_SEASONAL_ARTEFACT },
    { x: -1, y: 0, name: "Camel Bone" },
    { x: -2, y: 0, name: "Camel Bone" },
  ],

  ARTEFACT_SIX: [
    { x: 0, y: 0, name: CURRENT_SEASONAL_ARTEFACT },
    { x: -1, y: 0, name: "Camel Bone" },
    { x: -2, y: -1, name: "Camel Bone" },
  ],

  ARTEFACT_SEVEN: [
    { x: 0, y: 0, name: CURRENT_SEASONAL_ARTEFACT },
    { x: 1, y: 1, name: "Camel Bone" },
  ],

  ARTEFACT_EIGHT: [
    { x: 0, y: 0, name: CURRENT_SEASONAL_ARTEFACT },
    { x: 1, y: 0, name: "Camel Bone" },
    { x: 0, y: 2, name: "Camel Bone" },
  ],

  ARTEFACT_NINE: [
    { x: 0, y: 0, name: CURRENT_SEASONAL_ARTEFACT },
    { x: 0, y: 1, name: "Camel Bone" },
    { x: 0, y: -1, name: "Camel Bone" },
    { x: -1, y: -1, name: "Camel Bone" },
  ],

  ARTEFACT_TEN: [
    { x: 0, y: 0, name: CURRENT_SEASONAL_ARTEFACT },
    { x: 0, y: 1, name: "Camel Bone" },
    { x: -1, y: 0, name: "Camel Bone" },
    { x: -1, y: 1, name: "Camel Bone" },
  ],

  ARTEFACT_ELEVEN: [
    { x: 0, y: 0, name: CURRENT_SEASONAL_ARTEFACT },
    { x: 0, y: 1, name: "Camel Bone" },
    { x: 1, y: 1, name: "Camel Bone" },
  ],

  ARTEFACT_TWELVE: [
    { x: 0, y: 0, name: CURRENT_SEASONAL_ARTEFACT },
    { x: 0, y: 1, name: "Camel Bone" },
    { x: -1, y: 1, name: "Camel Bone" },
    { x: 1, y: 1, name: "Camel Bone" },
  ],

  ARTEFACT_THIRTEEN: [
    { x: 0, y: 0, name: CURRENT_SEASONAL_ARTEFACT },
    { x: 2, y: 0, name: "Camel Bone" },
  ],

  ARTEFACT_FOURTEEN: [
    { x: 0, y: 0, name: CURRENT_SEASONAL_ARTEFACT },
    { x: 0, y: 2, name: "Camel Bone" },
  ],

  ARTEFACT_FIFTEEN: [
    { x: 0, y: 0, name: "Camel Bone" },
    { x: 1, y: 0, name: CURRENT_SEASONAL_ARTEFACT },
    { x: 2, y: 0, name: "Camel Bone" },
  ],

  ARTEFACT_SIXTEEN: [
    { x: 0, y: 0, name: CURRENT_SEASONAL_ARTEFACT },
    { x: 1, y: 0, name: "Camel Bone" },
    { x: 1, y: 1, name: "Camel Bone" },
  ],

  ARTEFACT_SEVENTEEN: [
    { x: 0, y: 0, name: "Camel Bone" },
    { x: 1, y: 0, name: "Camel Bone" },
    { x: 1, y: 1, name: CURRENT_SEASONAL_ARTEFACT },
  ],

  ARTEFACT_EIGHTEEN: [
    { x: 0, y: 0, name: CURRENT_SEASONAL_ARTEFACT },
    { x: 1, y: 0, name: "Camel Bone" },
    { x: 0, y: 1, name: "Camel Bone" },
    { x: 1, y: 1, name: "Camel Bone" },
  ],

  ARTEFACT_NINETEEN: [
    { x: 0, y: 0, name: "Camel Bone" },
    { x: 1, y: 0, name: "Camel Bone" },
    { x: 2, y: 0, name: "Camel Bone" },
    { x: 1, y: 1, name: CURRENT_SEASONAL_ARTEFACT },
  ],

  ARTEFACT_TWENTY: [
    { x: 0, y: 0, name: "Camel Bone" },
    { x: 1, y: 0, name: "Camel Bone" },
    { x: 0, y: 1, name: CURRENT_SEASONAL_ARTEFACT },
    { x: 1, y: 1, name: "Camel Bone" },
  ],

  ARTEFACT_TWENTY_ONE: [
    { x: 0, y: 0, name: CURRENT_SEASONAL_ARTEFACT },
    { x: 1, y: 0, name: "Camel Bone" },
    { x: 2, y: 0, name: "Camel Bone" },
    { x: 0, y: 1, name: "Camel Bone" },
    { x: 1, y: 1, name: "Camel Bone" },
  ],

  ARTEFACT_TWENTY_TWO: [
    { x: 0, y: 0, name: "Camel Bone" },
    { x: 1, y: 0, name: "Camel Bone" },
    { x: 2, y: 0, name: "Camel Bone" },
    { x: 0, y: 1, name: "Camel Bone" },
    { x: 1, y: 1, name: CURRENT_SEASONAL_ARTEFACT },
  ],

  ARTEFACT_TWENTY_THREE: [
    { x: 0, y: 0, name: "Camel Bone" },
    { x: 1, y: 0, name: "Camel Bone" },
    { x: 2, y: 0, name: "Camel Bone" },
    { x: 1, y: 1, name: "Camel Bone" },
    { x: 2, y: 1, name: CURRENT_SEASONAL_ARTEFACT },
  ],

  ARTEFACT_TWENTY_FOUR: [
    { x: 0, y: 0, name: "Camel Bone" },
    { x: 1, y: 0, name: CURRENT_SEASONAL_ARTEFACT },
    { x: 2, y: 0, name: "Camel Bone" },
    { x: 0, y: 1, name: "Camel Bone" },
    { x: 2, y: 1, name: "Camel Bone" },
  ],

  // Small L - X Coins
  HIEROGLYPH: [
    { x: 0, y: 0, name: "Vase" },
    { x: 1, y: 0, name: "Vase" },
    { x: 0, y: 1, name: "Hieroglyph" },
  ],

  // Square - X Coins
  OLD_BOTTLE: [
    { x: 0, y: 0, name: "Old Bottle" },
    { x: 1, y: 0, name: "Old Bottle" },
    { x: 0, y: 1, name: "Old Bottle" },
    { x: 1, y: 1, name: "Old Bottle" },
  ],

  // Diagonal - X Coins
  COCKLE: [
    { x: 0, y: 0, name: "Cockle Shell" },
    { x: 1, y: 1, name: "Cockle Shell" },
    { x: 2, y: 2, name: "Cockle Shell" },
  ],

  // Horizontal - X Coins
  WOODEN_COMPASS: [
    { x: 0, y: 0, name: "Wood" },
    { x: 1, y: 0, name: "Wooden Compass" },
    { x: 2, y: 0, name: "Wood" },
  ],

  SEA_CUCUMBERS: [
    { x: 0, y: 0, name: "Sea Cucumber" },
    { x: 1, y: 0, name: "Sea Cucumber" },
    { x: 2, y: 0, name: "Sea Cucumber" },
    { x: 3, y: 0, name: "Pipi" },
  ],

  SEAWEED: [
    { x: 0, y: 0, name: "Seaweed" },
    { x: 1, y: 0, name: "Seaweed" },
    { x: 2, y: 0, name: "Seaweed" },
    { x: 2, y: 1, name: "Starfish" },
  ],

  CLAM_SHELLS: [
    { x: 0, y: 0, name: "Clam Shell" },
    { x: 1, y: 0, name: "Clam Shell" },
    { x: 0, y: -1, name: "Clam Shell" },
    { x: 1, y: -1, name: "Clam Shell" },
  ],

  CORAL: [
    { x: 0, y: 1, name: "Stone" },
    { x: 0, y: 0, name: "Coral" },
    { x: 0, y: -1, name: "Stone" },
  ],
  PEARL: [
    { x: 0, y: 1, name: "Stone" },
    { x: 0, y: 0, name: "Pearl" },
    { x: 0, y: -1, name: "Stone" },
  ],
  PIRATE_BOUNTY: [{ x: 0, y: 0, name: "Pirate Bounty" }],
}

