...
async function handleExport () {
  try {
    const encodedState = encodeGridState(grid, landId)
    const success = await copyToClipboard(encodedState)
    
    if (success) {
      // Show success feedback
      showToast('Grid state copied to clipboard! 📋', 'success')
    } else {
      showToast('Failed to copy to clipboard', 'error')
    }
  } catch (error) {
    console.error('Export failed:', error)
    showToast('Failed to export grid state', 'error')
  }
}
...
async function handleImport () {
  try {
    ...
    if (navigator.clipboard && window.isSecureContext) {
      ...
      clipboardText = prompt('Paste the grid state code here:')
      if (!clipboardText) return
    }

    if (!isValidEncodedState(clipboardText)) {
      showToast('Invalid grid state data', 'error')
      return
    }

    const state = decodeGridState(clipboardText)
    const success = importMarksToGrid(state, grid)
    
    if (success) {
      showToast('Grid state imported successfully! ✅', 'success')
    } else {
      showToast('Failed to import grid state', 'error')
    }
  } catch (error) {
